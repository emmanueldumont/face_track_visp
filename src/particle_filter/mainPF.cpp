...
State_ state_tracking(StateData& d)
{
   ...
   if( confidence > 0.1 )
   {
      d.estimRect = d.filter.draw_estimated_state(d.image, target_size, GREEN);
   }
   else if( confidence > 0.025 )
   {
      d.estimRect = d.filter.draw_estimated_state(d.image, target_size, YELLOW);
   }
   else
   {
      d.estimRect = d.filter.draw_estimated_state(d.image, target_size, RED);
   }
   
   //cout << "Rect: (" << d.estimRect.x << ", " << d.estimRect.y << ", " << d.estimRect.width << ", " << d.estimRect.height  << ")" << endl;
...
{
  if(d.lbp.empty())
      d.lbp = Mat::zeros(frame.rows, frame.cols, CV_8UC1);
  //if(d.image.empty())
  frame.copyTo(d.image);
  
  //cout << "state_initializing: (" << d.selection.x << ", " << d.selection.y << ", " << d.selection.width << ", " << d.selection.height  << ")" << endl;
  
  d.selection = selectROI;
  d.estimRect = d.selection;
  
  //cout << "selection: (" << d.selection.x << ", " << d.selection.y << ", " << d.selection.width << ", " << d.selection.height  << ")" << endl;
  state = state_initializing(d); // Call with current frame
}
...
int main(int argc, char** argv)
{
   ...
   for(;;)
   {
      ...
      // Start timing the loop -Manu: not needed
      //timeval start_time;
